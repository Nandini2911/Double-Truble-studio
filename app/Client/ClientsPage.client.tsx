...
const LOGOS = Array.from({ length: 32 }, (_, i) => {
  const n = i + 1;
  return { src: `/logos/${n}.webp`, alt: `Client ${n}` };
});
    ...
    >
      ...
      <div className="relative mx-auto w-full max-w-6xl 2xl:max-w-[1500px] px-4 sm:px-6 lg:px-8 2xl:px-32 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-muted">
          We’re proud to collaborate with leading brands and visionary companies driving innovation across industries.
        </p>
        ...
        >
          {LOGOS.map((logo, index) => (
            <div
              key={logo.src}
              className="
                group relative rounded-2xl border border-black/10 bg-white
                px-4 py-4 sm:px-5 sm:py-5 transition
                hover:border-black/15 hover:shadow-[0_10px_30px_rgba(0,0,0,0.18)]
                before:pointer-events-none before:absolute before:inset-0 before:rounded-2xl
                before:opacity-0 before:transition-opacity before:duration-300
                group-hover:before:opacity-100
                before:bg-[linear-gradient(120deg,rgba(70,243,216,0.10),rgba(255,63,164,0.08),rgba(216,184,115,0.06))]
              "
            >
              <div className="relative flex h-14 sm:h-16 md:h-20 items-center justify-center">
                <Image
                  src={logo.src}
                  alt={logo.alt}
                  width={160}
                  ...
                  className="
                    ...
                    object-contain opacity-85 transition
